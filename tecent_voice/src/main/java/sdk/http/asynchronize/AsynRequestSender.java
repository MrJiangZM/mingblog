 ...
 */
package sdk.http.asynchronize;

import sdk.http.base.HttpRequester;
...
import static sdk.config.AsrGlobelConfig.CUT_LENGTH;
...
public class AsynRequestSender {
    ...
    private void sendEndBytes(RasrBytesRequest rasrBytesRequest, byte[] endBytes) {
        ...
        VoiceResponse voiceResponse = this.sendRequest(StringTools.getRandomString(16), rasrBytesRequest, 1, endBytes);
     ...
     *
     * @param canSendLast 如果最后一块刚好等于CUT_LENGTH时，是否可以发出。如果为false，则即使最后一块的长度刚好等于CUT_LENGTH，也不会切分。
     *
    ...
    private void sendMiddleCut(RasrBytesRequest rasrBytesRequest, byte[] current, int posi) {
        ...
        VoiceResponse voiceResponse = this.sendRequest(StringTools.getRandomString(16), rasrBytesRequest, 0, cut);
    ...
    private VoiceResponse sendRequest(String voiceId, RasrBytesRequest rasrBytesRequest, int end, byte[] dataPacket) {
        rasrBytesRequest.updateSeqAndEndFlag(this.seq, end);
        VoiceResponse voiceResponse = this.httpRequester.sendAndParse(voiceId, rasrBytesRequest, dataPacket);
        if (voiceResponse != null)
