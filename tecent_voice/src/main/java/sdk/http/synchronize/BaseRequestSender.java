...
public class BaseRequestSender {
	...
	public VoiceResponse send(String voiceId, RasrBaseRequest rasrBasesRequest, InputStream inputStream) {
		...
		try {
			...
			while (avaiable > 0) {
				...
				voiceResponse = this.httpRequester.sendAndParse(voiceId, rasrBasesRequest, dataPacket);
				seq++;
			}
			httpclient.close();
		} catch (Exception e) {
