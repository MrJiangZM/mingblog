...
public class SpringExecutorConfig<V> {
    ...
    @Bean(name = "serviceTaskExecutor1")
...
//        executor.initialize();        //初始化执行器
    ...
    @Bean(name = "serviceTaskExecutor2")
    public ThreadPoolTaskExecutor serviceTaskExecutor2() {
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        executor.setCorePoolSize(CORE_POOL_SIZE);        //配置核心线程数
//        executor.setMaxPoolSize(maxPoolSize);        //配置最大线程数 ， 使用默认integer.max
    ...
    public Executor serviceTaskExecutor3() {
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        executor.setCorePoolSize(CORE_POOL_SIZE);        //配置核心线程数
//        executor.setMaxPoolSize(maxPoolSize);        //配置最大线程数 ， 使用默认integer.max
        executor.setQueueCapacity(QUEUE_CAPACITY);        //配置队列大小
    ...
    @Bean
    public CompletionService complete(@Qualifier("serviceTaskExecutor2") ThreadPoolTaskExecutor serviceTaskExecutor2) {
        return new ExecutorCompletionService<V>(serviceTaskExecutor2);
    }
