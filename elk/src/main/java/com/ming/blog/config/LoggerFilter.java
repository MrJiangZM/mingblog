...
public class LoggerFilter extends OncePerRequestFilter {
    ...
    private static final List<String> HEAD_NOT_INCLUDE = Arrays.asList("Accept", "Accept-Encoding", "Accept-Charset",
            ...
            "content-type", "vary", "cache-control", "cookie", "host", "keep-alive");
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            response = new ResponseWrapper(requestId, System.currentTimeMillis(), response);
    ...
    private void logRequest(final HttpServletRequest request) {
        ...
        if (request.getMethod() != null) {
            ...
            if (!StringUtils.equalsIgnoreCase(METHOD_OPTIONS, method)) {
        ...
        if (request instanceof RequestWrapper && !isMultipart(request) && !isBinaryContent(request)) {
            ...
            try {
                String charEncoding = requestWrapper.getCharacterEncoding() != null ? requestWrapper.getCharacterEncoding() : StandardCharsets.UTF_8.name();
    ...
    private boolean isBinaryContent(final HttpServletRequest request) {
        return request.getContentType() != null && (request.getContentType().startsWith("image") || request.getContentType().startsWith("video")
    ...
    private void logResponse(final ResponseWrapper response) {
        StringBuilder msg =
                new StringBuilder().append(RESPONSE_PREFIX).append("ID: [").append((response.getId())).append("], Cost-time: [").append((System.currentTimeMillis() - response.getRequestTime()));
        try {
